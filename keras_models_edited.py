# from keras commit 4ecb5bdd14f664129f26d805e9f78c8dd41bba24
# edited because I'm doing something weird: unsupervised learning
# (on a massive dataset that doesn't even vaguely pretend to fit in ram)
...
class UnsupervisedWackySequential(Model, containers.Sequential):
    '''
    ...
    def compile(self, optimizer, loss, loading_func,
        ...
        train_loss = self.loss(self.y, self.y_train).flatten().mean()
        test_loss = self.loss(self.y, self.y_test).flatten().mean()
        ...
        def preprocess(function, deterministic=False):
            def wrapper(filenames):
                result = loading_func(filenames, deterministic)
    ...
    def fit(self, X, batch_size=3, nb_epoch=100, verbose=1, callbacks=[],
